    ...
    while fg:
        ...
        try:
            while len(row):
                ...
                if flag:
                    WebDriverWait(wd, 10).until(EC.presence_of_element_located((By.XPATH, '//div[starts-with(@class,"a-alert-content")]')))
                    wd.find_element_by_xpath('//input[@value="Delete"]').click()
                time.sleep(2)
                wd.execute_script('window.stop()')
                amazon_store.add_inventory(_id, price, review_count, grade_count, total_inventory, crawl_time, create_time, remark)
